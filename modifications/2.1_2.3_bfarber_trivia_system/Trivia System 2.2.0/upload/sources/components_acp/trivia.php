...
class ad_trivia {
	...
	*/
	...
	function show_splash()
	{
		...
		$percent_c = (intval($served['served']))?round((intval($correct['correct'])/intval($served['served']))*100,2):0;
		...
		if(count($topserved) > 0)
		{
			...
			{
				$percent_c = round((intval($to_out['correct'])/intval($to_out['served']))*100,2);
